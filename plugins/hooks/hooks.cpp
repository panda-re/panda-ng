...
SUPPORT_CALLBACK_TYPE(block_translate)
...
void enable_hooking() {
    assert(self != NULL);
    panda_enable_callback(self, PANDA_CB_BLOCK_TRANSLATE, block_translate_callback);
}

void disable_hooking() {
    assert(self != NULL);
    panda_disable_callback(self, PANDA_CB_BLOCK_TRANSLATE, block_translate_callback);
}
...
void before_block_translate_invalidator(CPUState* cpu, uint64_t pc_val){
...
void cb_block_translate_callback (CPUState* cpu, struct qemu_plugin_tb *tb) {
    ...
    for (auto& a : before_tcg_codegen_hooks){
        ...
        while(it != before_tcg_codegen_hooks[asid].end() && it->addr < tb_vaddr + tb_size){
            auto h = (hook*)&(*it);
            if (likely(h->enabled)){
                if (h->asid == asid){ 
                    if (h->km == MODE_ANY || (in_kernel && h->km == MODE_KERNEL_ONLY) || (!in_kernel && h->km == MODE_USER_ONLY)){
...
MAKE_HOOK_VOID(BEFORE_BLOCK_TRANSLATE, before_block_translate, (CPUState *cpu, uint64_t pc), panda_current_pc(cpu), cpu, pc, h)

MAKE_HOOK_VOID(AFTER_BLOCK_TRANSLATE, after_block_translate, (CPUState *cpu, TranslationBlock *tb), tb->pc, cpu, tb, h)

// MAKE_HOOK_VOID(BLOCK_TRANSLATE, block_translate, (CPUState *cpu, struct qemu_plugin_tb *tb), qemu_plugin_tb_vaddr(tb), cpu, tb, h)
...
bool init_plugin(void *_self) {
    ...
    REGISTER_AND_DISABLE_CALLBACK(_self, block_translate, BLOCK_TRANSLATE)
