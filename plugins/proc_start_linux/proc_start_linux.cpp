...
string read_str(CPUState* cpu, target_ulong ptr){
    ...
    char tmp;
    while (true){
        if (panda_virtual_memory_read(cpu, ptr, (unsigned char*)&tmp,1) == MEMTX_OK){
            buf += tmp;
            if (tmp == '\x00'){
                break;
            }
...
#define HF_CS64_SHIFT       15 /* only used on x86_64: 64 bit code segment  */
    if(sizeof(T) == 8) {
        CPUArchState *env = static_cast<CPUArchState *>(panda_cpu_env(cpu));
...
bool asid_changed(CPUState *env, uint64_t old_asid, uint64_t new_asid) {
...
bool init_plugin(void *self) {
    self_ptr = self;
    #if defined(TARGET_PPC) || defined(TARGET_RISCV64) || defined(TARGET_RISCV32) || defined(TARGET_LOONGARCH)
