...
void load_syscall_info(const gchar *arch, syscall_info_t **syscall_info, syscall_meta_t **syscall_meta) {
    ...
    if (panda_os_familyno == OS_WINDOWS) {
    	...
    	if (0 == strncmp(panda_os_variant, "7", 1)) {
    	    syscall_info_dlname = g_strdup_printf("%s/lib%s_dso_info_%s_7_%s" ".so", PLUGIN_NAME, PLUGIN_NAME, panda_os_family, arch);
    	} else {
            // for windows, take into account the panda_os_variant
            syscall_info_dlname = g_strdup_printf("%s/lib%s_dso_info_%s_%s_%s" ".so", PLUGIN_NAME, PLUGIN_NAME, panda_os_family, panda_os_variant, arch);
    	}
    }
    else {
        ...
        syscall_info_dlname = g_strdup_printf("%s/lib%s_dso_info_%s_%s" ".so", PLUGIN_NAME, PLUGIN_NAME, panda_os_family, arch);
    }
