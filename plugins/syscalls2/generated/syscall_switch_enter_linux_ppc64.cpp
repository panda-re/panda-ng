...
void syscall_enter_switch_linux_ppc64(CPUState *cpu, int profile, target_ptr_t pc, int static_callno) {
	...
	// 291 long sys_newfstatat ['int dfd', 'const char __user *filename', 'struct stat __user *statbuf', 'int flag']
	case 291: {
		panda_noreturn = false;
		ctx.double_return = false;
		int32_t arg0 = get_s32(cpu, &ctx, 0);
	...
	// 392 long sys_semtimedop ['int semid', 'struct sembuf __user *sops', 'unsigned nsops', 'const struct timespec __user *timeout']
	case 392: {
		panda_noreturn = false;
		ctx.double_return = false;
		int32_t arg0 = get_s32(cpu, &ctx, 0);
